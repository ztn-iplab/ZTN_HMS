document.addEventListener("DOMContentLoaded", () => {
  ...
  async function triggerWebAuthnFlow(password) {
    try {
      const challengeRes = await fetch("/auth/reset-webauthn-begin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!challengeRes.ok)
        throw new Error(beginData.error || "Failed to begin WebAuthn reset.");
      ...
      const verifyRes = await fetch("/auth/reset-webauthn-complete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!retry.ok)
        throw new Error(retryResult.error || "Final TOTP reset failed.");
